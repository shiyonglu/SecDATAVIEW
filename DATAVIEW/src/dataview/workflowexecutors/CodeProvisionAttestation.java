...
import dataview.workflowexecutors.SHA256Hash;
...
//	private VMProvisionerOriginal vmProvisioner;
	...
	boolean isCodeProvisionOkay(ArrayList<String> IPs) {
		//runCodeProvision();
	...
	public void encryptTaskExecutorJar(String workflowLibDir) {
		...
		try {
			...
			for (int i = 0; i < IPs.size(); i++) {
				if (ipPool.get(IPs.get(i)).equals(WorkflowExecutorAlpha.SGX)) {
					sgxProvisioner.copyFileVM(workflowLibDir + "/TaskExecutor.enc", destinationFolder, IPs.get(i));
				} else if (ipPool.get(IPs.get(i)).equals(WorkflowExecutorAlpha.AMD)) {
					amdProvisioner.copyFileVM(workflowLibDir + "/TaskExecutor.enc", destinationFolder, IPs.get(i));
				} else if (ipPool.get(IPs.get(i)).equals(WorkflowExecutorAlpha.AWS)) {
		...
		for (int i = 0; i < IPs.size(); i++) {
			...
			while (itr.hasNext()) {
				try {
					...
					if (ipPool.get(IPs.get(i)).equals(WorkflowExecutorAlpha.SGX)) {
						sgxProvisioner.copyFileVM(file, destinationFolder, IPs.get(i));
					} else if (ipPool.get(IPs.get(i)).equals(WorkflowExecutorAlpha.AMD)) {
						amdProvisioner.copyFileVM(file, destinationFolder, IPs.get(i));
					} else if (ipPool.get(IPs.get(i)).equals(WorkflowExecutorAlpha.AWS)) {
						awsProvisioner.copyFileVM(file, destinationFolder, IPs.get(i));
					}
...
//							sendingFilestoVM(vmProvisioner, destinationFolder);
						...
						if (messages[0].equalsIgnoreCase("decryptionComplete")) {
							...
							if (ipPool.get(IPs.get(vm)).equals(WorkflowExecutorAlpha.SGX)) {
							...
							} else if (ipPool.get(IPs.get(vm)).equals(WorkflowExecutorAlpha.AMD)) {
								...
								Dataview.debugger.logSuccessfulMessage("File transfer is completed for clientkeystore");
							} else if (ipPool.get(IPs.get(vm)).equals(WorkflowExecutorAlpha.AWS)) {
